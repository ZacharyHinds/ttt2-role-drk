...
if CLIENT then
  ...
  function HUDELEMENT:Draw()
    ...
    local color = DRUNK.color
    if not color then print("[TTT2 Drunk] No Color Found") return end

    if not client:IsActive() or not client:Alive() or client:GetSubRole() ~= ROLE_DRUNK then return end
    local start_time = client:GetNWFloat("ttt2_drk_start_time", 0)
    local raw_rolestring = client:GetNWString("ttt2_drk_rolestring")
    local rolestring = LANG.TryTranslation(raw_rolestring)
    local delay = client:GetNWInt("ttt2_drk_delay", 0)
    -- print("[TTT2 Drunk]",start_time,raw_rolestring,rolestring,delay)
    if start_time + delay  > CurTime() then
      multiplier = (CurTime() - start_time) / delay

      local secondColor = DRUNK.bgcolor
    ...
    elseif multiplier and multiplier > 0 then
      self:DrawComponent(multiplier, color, LANG.GetParamTranslation("ttt2_drk_sobering", {role = rolestring}) or nil)
    end
  end
end
